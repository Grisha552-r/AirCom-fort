...
export default function ProductsListView({
  ...
  useEffect(() => {
    ...
    const brand = SLUG_TO_BRAND[initialCategory];
    if (brand) setSelectedBrands(new Set([brand]));
    const type = SLUG_TO_TYPE[initialCategory];
    if (type) setSelectedTypes(new Set([type]));
    if (initialCategory === 'split-systems') setSelectedTypes(new Set(['wall']));
  }, [initialCategory]);
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products;
  }, [allProducts, initialSearch, selectedBrands, selectedTypes, selectedBtus, inverterFilter, priceMin, priceMax, inStockOnly, sortBy]);
      ...
      <section className="max-w-7xl mx-auto px-4 py-10">
        ...
        <div className="space-y-3 max-w-3xl">
          {[
            ...
            ['Какая гарантия на кондиционер?', 'Electrolux и LG — 3 года, Ballu и Haier — 2 года, Mitsudai и King Home — 1 год, Vicool VIERA — 3 года, Vicool PRO Light — 5 лет. На монтажные работы — отдельная гарантия 1 год. Гарантийный ремонт выполняем бесплатно с выездом мастера.'],
