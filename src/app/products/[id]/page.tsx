...
const CATEGORY_META: Record<string, { label: string; brand?: string; categoryIds?: string[] }> = {
  'split-systems':  { label: 'Все сплит-системы',         categoryIds: WALL_CATEGORY_IDS },
  'split-electrolux': { label: 'Кондиционеры Electrolux', brand: 'Electrolux' },
  'split-ballu':    { label: 'Кондиционеры Ballu',         brand: 'Ballu' },
  'split-haier':    { label: 'Кондиционеры Haier',         brand: 'Haier' },
  'split-lg':       { label: 'Кондиционеры LG',            brand: 'LG' },
  'split-mitsudai': { label: 'Кондиционеры Mitsudai',      brand: 'Mitsudai' },
  'mobile':         { label: 'Мобильные кондиционеры',     categoryIds: ['mobile'] },
};
  ...
  useEffect(() => {
    ...
    fetch('/api/products')
      ...
      .then((all: Product[]) => {
        ...
        if (desc) desc.setAttribute('content', `Купить ${found.name} в Гомеле по цене ${found.price.toLocaleString('ru-RU')} р. Официальная гарантия. Установка под ключ.`);
        ...
        crumbs.push({ name: found.name, url: `https://aircom-fort.by/products/${found.id}` });
          ...
          '@graph': [
            {
              ...
              offers: {
                ...
                url: `https://aircom-fort.by/products/${found.id}`,
